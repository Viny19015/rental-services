const equipmentList = document.querySelector('.equipment ul');
const seedsList = document.querySelector('.seeds ul');

for (let i = 1; i <= 10; i++) {
    const equipmentItem = document.createElement('li');
    equipmentItem.textContent = `Equipment ${i}`;
    equipmentList.appendChild(equipmentItem);

    const seedsItem = document.createElement('li');
    seedsItem.textContent = `Seeds ${i}`;
    seedsList.appendChild(seedsItem);
}
